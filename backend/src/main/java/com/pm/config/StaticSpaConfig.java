 ...
 * app runs on a single port (used by the tray / "app-like" mode).
 *
 * Gated on {@code pm.web.dist}: the normal dev flow (start-dev.cmd, Vite on
 * 5180) never sets this property, so the backend keeps serving only /api and
 * dev behaviour is completely unchanged.
 */
@Configuration
@ConditionalOnProperty(name = "pm.web.dist")
